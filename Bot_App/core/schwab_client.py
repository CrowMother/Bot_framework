...
class SchwabClient:
    """Thin wrapper around ``schwabdev.Client`` with retry logic."""

    def __init__(self, app_key, app_secret):
        """Instantiate the client using the provided credentials."""

        self.client = Client(app_key, app_secret)

    def get_account_positions(self, status_filter="FILLED", hours=1):
        """Return account orders from the last ``hours`` filtered by status."""

        def fetch_orders():
            from_time = get_end_time(hours)
            to_time = get_start_time(hours)
            return self.client.account_orders_all(
                from_time, to_time, None, status_filter
            )
        ...
        if response is not None and response.status_code == 200:
            ...
            logging.error(
                f"Failed to get positions after retries. Response: {response}"
            )
            return []
